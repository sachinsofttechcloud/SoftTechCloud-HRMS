...
function anniversaryLabel(joiningDate) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(ymd)) return "Less than 1 year";
    const [joinYear, joinMonth, joinDay] = ymd.split("-").map(Number);
    const now = new Date();
    let years = now.getFullYear() - joinYear;
    const month = now.getMonth() + 1;
    const day = now.getDate();
    if (month < joinMonth || (month === joinMonth && day < joinDay)) years -= 1;
    if (years < 1) return "Less than 1 year";
    return years === 1 ? "1 year completed" : `${years} years completed`;
}
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-1 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 shadow-2xl flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    {user?.employeeId ? (
                        <p className="text-[11px] font-mono text-slate-400 mt-1">{user?.role || "EMPLOYEE"}</p>
                    ) : null}
                    ...
                    <div className="w-full mt-6 pt-5 border-t border-white/10 space-y-3.5 text-left text-xs">
                        ...
                        <div className="flex items-center justify-between text-slate-300">
                            ...
                            <span className="font-medium text-blue-300 font-mono">{user?.employeeId || "—"}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-slate-300">
                            <span className="text-slate-400 flex items-center gap-1.5">
                                <Calendar size={14} className="text-pink-400" /> Birth date
                            </span>
                        ...
                        <div className="flex items-center justify-between text-slate-300">
                            <span className="text-slate-400 flex items-center gap-1.5">
                                <Calendar size={14} className="text-amber-400" /> Joining date
                            </span>
                            <span className="font-medium text-slate-200">
                                {user?.joiningDate
                                    ? String(user.joiningDate).slice(0, 10).split("-").reverse().join("/")
                                    : "—"}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-slate-300">
                            <span className="text-slate-400 flex items-center gap-1.5">
                                <Sparkles size={14} className="text-amber-400" /> Work anniversary
                            </span>
                            <span className="font-medium text-amber-200">
                                {user?.workAnniversaryLabel
                                    || anniversaryLabel(user?.joiningDate)}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <span className="text-[10px] xl:text-[12px] font-semibold text-red-300">{user?.bloodGroup || "O+"}</span>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 shadow-xl space-y-4">
                        ...
                        {user?.education ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Master&apos;s / Degree</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white">{user.education.degree}</span>
                                </div>
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Institute Name</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white">{user.education.instituteName}</span>
                                </div>
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Passing Year</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white">{user.education.passingYear}</span>
                                </div>
                                {/* <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Linked Employee ID</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-blue-300 font-mono">
                                        {user.education.employeeId || user.employeeId || "—"}
                                    </span>
                                </div> */}
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    ...
                                    {user.education.certificate ? (
                                        ...
                                        <span className="text-[10px] xl:text-[12px] font-semibold text-slate-400">Not uploaded</span>
                                    )}
                                </div>
                            </div>
                        ) : (
                            <p className="text-xs text-slate-400 italic">Education details were not recorded during onboarding.</p>
                        )}
                    </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 shadow-xl space-y-4">
                        <div className="flex items-center justify-between border-b border-white/10 pb-3">
                            ...
                            <span className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Not editable</span>
                        </div>
                        {user?.bankDetail ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Account Number</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white font-mono">{user.bankDetail.accountNumber}</span>
                                </div>
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Account Type</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white">{user.bankDetail.accountType}</span>
                                </div>
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">IFSC Code</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white font-mono">{user.bankDetail.ifscCode}</span>
                                </div>
                                <div className="bg-black/30 border border-white/5 rounded-xl p-3.5">
                                    <span className="text-slate-400 block mb-1">Branch Name</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-white">{user.bankDetail.branchName}</span>
                                </div>
                                {/* <div className="bg-black/30 border border-white/5 rounded-xl p-3.5 sm:col-span-2">
                                    <span className="text-slate-400 block mb-1">Linked Employee ID</span>
                                    <span className="text-[10px] xl:text-[12px] font-semibold text-blue-300 font-mono">
                                        {user.bankDetail.employeeId || user.employeeId || "—"}
                                    </span>
                                </div> */}
                            </div>
                        ) : (
                            <p className="text-xs text-slate-400 italic">Bank details were not recorded during onboarding.</p>
                        )}
                    </div>
                </div>
            </div>
